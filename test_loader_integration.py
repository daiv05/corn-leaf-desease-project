...
def test_dataset_pipeline():
    ...
    if not os.path.exists(csv_train_path):
        print(f"❌ Error: No se encuentra el manifiesto '{csv_train_path}'. Ejecuta primero scripts/create_splits.py")
        return
    ...
    try:
        ...
        print(f"   🏷️ Índice de clase mapeado numéricamente: {sample_label} (Corresponde a: '{dataset.idx_to_class[sample_label]}')")
        print(f"   📈 Valores espectrales del tensor -> Mínimo: {sample_tensor.min():.4f} | Máximo: {sample_tensor.max():.4f}")
        ...
        print(f"   📦 Solicitando un lote optimizado de {batch_size} imágenes a la cola de memoria...")
        images_batch, labels_batch = next(iter(data_loader))
        
        print("\n" + "=" * 60)
        print("🎉 ¡DIAGNÓSTICO COMPLETADO EXITOSAMENTE!")
        print("=" * 60)
        print(f"   Tensor de imágenes del lote (Batch Size, Canales, Alto, Ancho): {images_batch.shape}")
        print(f"   Tensor de etiquetas correspondientes del lote: {labels_batch}")
        print("   El flujo de datos está 100% libre de colisiones dimensionales y listo para entrenar.")
        print("=" * 60)
