...
def run_data_preparation_pipeline(config_path: str) -> None:
    ...
    output_dir = config['paths']['split_output_dir']
        ...
        for environment in os.listdir(class_path):
            ...
            for img_name in os.listdir(env_path):
                if img_name.lower().endswith(('.png', '.jpg', '.jpeg')):
                    raw_image_paths.append((class_name, environment, os.path.join(env_path, img_name)))
    ...
    for class_name, environment, img_path in tqdm(raw_image_paths, desc="Procesando imágenes de maíz", unit="img"):
        try:
            # Calcular huella digital visual (pHash) en caliente
            img_hash = hasher.calculate_phash(img_path)
            ...
            # Añadir registro limpio al inventario
            all_records.append({
                'image_path': img_path,
                'label': class_name,
            ...
            tqdm.write(f"⚠️ Saltando archivo dañado o inválido en {img_path}: {str(e)}")
    ...
    report_dir = os.path.join("dataset", "reports", "class_distribution")
